...
def group_products_by_category(products):
    try:
        ...
        for dict in products:
            if "category" in dict:
                if (dict["category"] not in list_category):
                    list_category.append(dict["category"])
        for category in list_category:
            list_product = []
            for dict in products:
                if "category" in dict:
                    if dict["category"] == category:
                        list_product.append(dict["name"])
                        dict_products.update({category: list_product})
        return dict_products
    except KeyError:
        return dict_products


print(group_products_by_category([{"name": "Apple", "price": 120, "quantity": 10},
                                  {"name": "Banana", "category": "fruit", "price": 90, "quantity": 15}]))
